...
class WeChatSessionConfig:
    ...
    ydxmid: str = "2"         # 项目 ID（示例：1=羽毛球, 2=乒乓球，具体以抓包为准）
...
def solve_captcha_by_bruteforce_0_20(
    ...
    for v in range(0, 21):
        ...
        # 成功判定：id 非空（最可靠）
        if rid and rid != "0":
            return v, f"BRUTE_OK: attempts={len(last_states)}/21 yzm={v} state={state} id={rid}"

        # 经验判定：有些实现成功时 state=0/true 且带 id；这里保守，只要 state 看起来不是“验证码错误”就停止交给上层处理
        if state not in {"40", "NON_JSON"}:
            return v, f"BRUTE_STOP: attempts={len(last_states)}/21 yzm={v} state={state} id={rid}"

        time.sleep(interval_ms / 1000.0)

    return None, "BRUTE_FAIL: attempts=21/21 last=" + str(last_states[-3:])
...
def choose_best_select_id(
    *,
    ...
    for it in avail_items:
        ...
        if not sid or sid in booked_ids:
            continue
        try:
            sd = int(it.get("sd"))
        except Exception:
...
def main() -> int:
    ...
    if len(sys.argv) >= 2 and sys.argv[1].lower() in {"wait6", "at6"}:
        ...
        log_info(f"启用 6 点定时：当前时间={now.strftime('%Y-%m-%d %H:%M:%S')}，将等待 {int(wait_seconds)} 秒在 {target.strftime('%Y-%m-%d %H:%M:%S')} 开始抢号")
        # 分段 sleep，方便中途 Ctrl+C 终止
        step = 30
    ...
    try:
        ...
        # 0) 执行预约前：查询“绿色可预约格子数量”，并自动选择目标格子（优先 20:00-21:00，其次 18:00-19:00）
        log_info("开始自动选择目标格子（优先 20:00-21:00，其次 18:00-19:00）")
        kyy = list_available_cells(
            s, cgid=cfg.cgid, ydxmid=cfg.ydxmid, days=cfg.days, log_request=False
        )
        avail = [c for c in kyy if c.get("ygq") != "1"]
        ...
        log_info(f"当前绿色可预约格子数量={len(green_ids)}")

        best = choose_best_select_id(avail_items=avail, booked_ids=booked_ids, prefer_sd=[20, 18])

        second_url = cfg.referer
        if best:
            cfg.cdid = best
            cfg.referer = update_referer_select_id(cfg.referer, best)
            second_url = cfg.referer
            s.headers["Referer"] = cfg.referer
            try:
                sd_s, court_s = cfg.cdid.split("_", 1)
                sd_i = int(sd_s)
                court_i = int(court_s)
                log_info(f"正在尝试抢：{court_i}号场 {sd_to_timerange(sd_i)}（cdid={cfg.cdid}）")
            except Exception:
                log_info(f"正在尝试抢：cdid={cfg.cdid}（无法解析场地/时段）")
            log_info("referer_select_id 已同步")
        else:
            log_warn("当前未找到可预约格子，将继续按 cfg.cdid 尝试提交（可能失败）")

        # 1) 打开 second.aspx
        r = http_get(s, second_url, timeout=20)
        r.raise_for_status()
        if not r.encoding or r.encoding.lower() in {"iso-8859-1", "ascii"}:
            r.encoding = r.apparent_encoding
        html = r.text
        web_state = extract_webforms_state(html)
        log_info("webforms_state_keys=" + (",".join(sorted(web_state.keys())) or "(none)"))

        hints = ["验证码", "预约信息", "下一步", "提交", "场馆", "时段"]
        hit = [h for h in hints if h in html]
        log_info("page_hints=" + (",".join(hit) or "(no-hints)"))

        # 2) 获取验证码图片（只保存在内存里，先不落盘，减少磁盘 IO 对热路径的影响）
        captcha_url = f"{BASE}/yzm/ValidCode.aspx"
        img_bytes = download_captcha_image(s, captcha_url)

        # 3) 穷举验证码（只输出总次数）
        yzm_value, msg = solve_captcha_by_bruteforce_0_20(s, cfg=cfg)
        log_info(f"captcha_solve={msg}")
        if yzm_value is None:
            log_error("无法在 0-20 内通过验证码校验，退出。")
            return 1

        # 4) 最终提交
        http_status, state, rid = submit_save_yy_data(
            s, cfg=cfg, yzm_value=yzm_value, log_request=True
        )
        log_info(f"最终提交结果: http_status={http_status} state={state} id={rid}")

        # 5) 预约流程全部结束后，再把验证码图片异步落盘（仅用于排查）
        try:
        ...
        if rid:
            log_info("预约成功（id 非空）。")
            return 0
        log_warn("预约未成功（id 为空）。请根据 state 进一步定位原因。")
        return 1
    finally:
